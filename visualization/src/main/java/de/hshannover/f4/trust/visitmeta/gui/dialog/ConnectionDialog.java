...
public class ConnectionDialog extends JFrame {
	...
	private JTabbedPane mJtPmain = null;

	private GuiController mContoller;

	private class MapServerConnection extends JPanel{

		private static final long serialVersionUID = -5995621652853572554L;
		...
		private void newLabels(){
			...
			mJlRawXml.setHorizontalAlignment(SwingConstants.RIGHT);
			mJlRawXml.setBounds(3, 80, 130, 16);
		}
		...
		private void newCheckBox(){
			...
			mJcBrawXML.setBounds(145, 76, 25, 25);
		}
		...
		private void addToPanels(){
			...
			mJpRight.add(mJlRawXml);
			mJpRight.add(mJcBdump);
			mJpRight.add(mJcBrawXML);
			mJpNorth.add(mJbAdd);
		...
		public MapServerConnection(RESTConnection[] cons) {
			...
			this.add(mJpContent, BorderLayout.CENTER);

			//			mJpContent.setBounds(100, 100, 671, 500);
			//			updateUI();
	...
	private class DataServiceConnection extends JPanel{
	...
	public ConnectionDialog(RESTConnection[] cons) {
		...
		newPanels();
		setContentPane(mJtPmain);
	}

	private void newPanels(){
		mJtPmain = new JTabbedPane();
		mJtPmain.add("Map Server Connections", new MapServerConnection(null));
	}
