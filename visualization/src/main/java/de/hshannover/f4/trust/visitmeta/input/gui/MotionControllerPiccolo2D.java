...
public class MotionControllerPiccolo2D implements MotionController {
	...
	public MotionControllerPiccolo2D(ConnectionTab tab) {
		...
		this.zfactor =
				mConfig.getDouble(VisualizationConfig.KEY_GUICONTROLLER_Z_FACTOR,
						VisualizationConfig.DEFAULT_VALUE_GUICONTROLLER_Z_FACTOR);
		this.xyfactor =
				mConfig.getDouble(VisualizationConfig.KEY_GUICONTROLLER_XY_FACTOR,
						VisualizationConfig.DEFAULT_VALUE_GUICONTROLLER_XY_FACTOR);
	}

	@Override
	public void moveCamera(double x, double y) {
		vCamera.translateView(x
				* xyfactor, y
						* xyfactor);
		logger.trace("moveCamera -> x: "
				+ x + ", y: ");
	}

	@Override
	public void zoom(double z) {
		vCamera.scaleViewAboutPoint(1
				- z
						* zfactor,
				vCamera.getViewBounds()
						.getCenterX(),
				vCamera.getViewBounds().getCenterY());
		logger.trace("zoom -> z: "
				+ z);
	}
	...
	public void setCursorPosition(double x, double y, double z) {
		...
		logger.trace("setCursorPosition -> x: "
				+ x + ", y: " + ", z: " + z);
	}
}
