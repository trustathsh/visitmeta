...
public class DefaultNodePainter implements NodePainter {
	...
	public DefaultNodePainter(GraphPanel panel) {
	...
	@Override
	public void paintMetadataNode(Metadata metadata, GraphicWrapper g) {
	...
	@Override
	public void paintIdentifierNode(Identifier identifier, GraphicWrapper g) {
		boolean isHighlighted = g.getStrokePaint().equals(mColorNewNode)
				|| g.getStrokePaint().equals(mColorDeleteNode);
		 ...
		 */
		g.setPaint(ColorHelper.getColor(g, identifier));
		g.setTextPaint(ColorHelper.getColorIdentifierText(identifier));

		if (!isHighlighted) {
			g.setStrokePaint(ColorHelper.getColorIdentifierStroke(identifier));
		}
	}
