			...
			if (in == null) {
				...
				for (InternalMetadata m : meta) {
					if(!in.hasMetadata(m)) {
						...
						if(m.isSingleValue()) {
							// remove old metadata (equals just compares typename for singleValue metadata)
							in.removeMetadata(m);
	...
	protected void submitDelete(InternalIdentifier id1, InternalIdentifier id2, List<InternalMetadata> meta) {
		...
		try {
			...
			if (linkToEdit != null) {
				...
				throw new RuntimeException("Someone is trying to delete from a link that is not in the graph or not connected to the given identifiers. Bad Boy!");
			}
			this.submitDelete(n);
	...
	protected void submitDelete(InternalIdentifier id, List<InternalMetadata> meta) {
		...
		try {
			...
			if (idGraph != null) {
				...
				throw new RuntimeException("Someone is trying to delete from an identifier that is not in the graph. Bad Boy!");
			}
			this.submitDelete(n);
