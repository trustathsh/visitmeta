 ...
 * 
 * This file is part of visitmeta-common, version 0.4.0,
...
public interface GraphService {
	/**
	 * @return A @ SortedMap} that maps timestamps to the number of changes that
	 *         occured at that time in acscending order. <i>Note: Due to the
	 *         nature of the IF-MAP protocol, this call is not idempotent!
	 *         Called at different points of time may yield varying results.</i>
	 */
	public SortedMap<Long, Long> getChangesMap();

	/**
	 * @return The oldest graph known in the application. I.e. the first update
	 *         the application received from the MAP server.
	 */
	public List<IdentifierGraph> getInitialGraph();

	/**
	 * @param timestamp
	 * @return State of the IF-MAP graph structure are time <tt>timestamp</tt>.
	 *         <i>Note: Due to the nature of the IF-MAP protocol, this call is
	 *         not idempotent! Called at different points of time may yield
	 *         varying results.</i>
	 */
	public List<IdentifierGraph> getGraphAt(long timestamp);

	/**
	 * @return The graph at the current time, i.e. the graph that represents the
	 ...
	 * @param filter
	 * @return The oldest graph known in the application. I.e. the first update
	 *         the application received from the MAP server. Filtered according
	 *         to the filter rules defined by the filter object.
	 */
	 ...
	 * @return State of the IF-MAP graph structure are time <tt>timestamp</tt>.
	 *         <i>Note: Due to the nature of the IF-MAP protocol, this call is
	 *         not idempotent! Called at different points of, it can yield
	 *         varying results.</i> Filtered according to the filter object.
	 */
	 ...
	 * @param filter
	 * @return The graph at the current time, i.e. the graph that represents the
	 *         latest changes in the IF-MAP graph structure. Filtered according
	 *         to the filter object.
	 */
	public List<IdentifierGraph> getCurrentGraph(GraphFilter filter);

	/**
	 * Calculates the changes in the IF-MAP graph structure that have occured
	 * between <tt>t1</tt> and <tt>t2</tt>.
	 * 
	 * @param t1
	 *            Point of time one (unix epoch)
	 * @param t2
	 *            Point of time two (unix epoch)
	 * @return {@link Delta} object containing updates and deletes in seperate
	 *         data structures.
	 */
	...
	/**
	 * Counts all entities of the given type in the graph at the given
	 * timestamp.
	 * 
	 * @param type
	 * @return
